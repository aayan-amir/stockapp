...
export default async function PrintSalesPage({ searchParams }) {
  ...
  const sales = await prisma.sale.findMany({
    where,
    orderBy: { txDate: 'desc' },
    include: { customer: true, items: { include: { stock: true } }, stock: true },
  })
    ...
    <>
      ...
      <div style={{ maxWidth: 960, margin: '32px auto', padding: '40px 48px', background: '#fff', boxShadow: '0 2px 24px rgba(0,0,0,.08)' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32 }}>
          <div>
            <div style={{ fontSize: 26, fontWeight: 800, color: '#1a1a1a', letterSpacing: -0.5 }}>SALES REPORT</div>
          ...
          <div style={{ textAlign: 'right' }}>
            ...
            <div style={{ fontSize: 13, color: '#7c3aed', fontWeight: 700, marginTop: 4 }}>{sales.length} record(s)</div>
          </div>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 16, marginBottom: 28 }}>
          {[['Total Sales', sales.length], ['Total Items Sold', totalQty]].map(([label, val]) => (
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          ...
          <tbody>
            {sales.length === 0 && (
              <tr><td colSpan={5} style={{ padding: '24px', textAlign: 'center', color: '#9ca3af', fontStyle: 'italic' }}>No sales found</td></tr>
            )}
            {sales.map((row, idx) => {
              ...
              return (
                <tr key={row.saleId} style={{ borderBottom: '1px solid #f3f4f6', background: idx % 2 === 1 ? '#fafafa' : '#fff' }}>
                  <td style={{ padding: '8px 10px', fontFamily: 'monospace', fontSize: 11, color: '#7c3aed', fontWeight: 600 }}>{row.invoiceNo || '—'}</td>
                  <td style={{ padding: '8px 10px', fontSize: 11, color: '#6b7280', whiteSpace: 'nowrap' }}>{fmtDate(row.txDate)}</td>
                  <td style={{ padding: '8px 10px', fontSize: 11, color: '#374151' }}>{row.customer?.customerName || 'Walk-in'}</td>
                  <td style={{ padding: '8px 10px', fontSize: 11, color: '#374151', maxWidth: 260 }}>{names}</td>
          ...
          <tfoot>
            <tr style={{ background: '#f8f5ff', fontWeight: 700 }}>
              <td colSpan={4} style={{ padding: '10px', textAlign: 'right', color: '#7c3aed', fontSize: 12 }}>Total</td>
        ...
        <div style={{ borderTop: '1px solid #e5e7eb', marginTop: 32, paddingTop: 16, display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#9ca3af' }}>
          ...
          <span>{sales.length} record(s) printed</span>
        </div>
      </div>
    </>
  )
