...
function StatCard({ label, value, sub, color = 'text-gold' }) {
  return (
    <div className="stat-card">
      ...
      <div className="text-slate-400 text-xs font-semibold uppercase tracking-widest mt-1">{label}</div>
      {sub && <div className="text-slate-300 text-xs mt-1">{sub}</div>}
    </div>
  )
}
    ...
    <div>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 card overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-800">
            ...
            <Link href="/ledger" className="text-accent text-xs hover:text-cyan-300 transition-colors">View all →</Link>
          </div>
          <div className="overflow-x-auto">
            <table className="data-table">
              ...
              <tbody>
                ...
                {recentTx.map(tx => (
                  <tr key={tx.saleId}>
                    <td className="font-mono text-xs text-accent">{tx.invoiceNo || '—'}</td>
                    <td><span className={tx.transactionType === 'Sale' ? 'badge-sale' : 'badge-purchase'}>{tx.transactionType}</span></td>
                    <td className="max-w-[140px] truncate text-slate-200">{tx.stock?.ourNo || '—'} {tx.stock?.description ? `· ${tx.stock.description}` : ''}</td>
                    <td className="text-slate-300 text-xs">{tx.transactionType === 'Sale' ? (tx.customer?.customerName || 'Walk-in') : (tx.supplierName || '—')}</td>
                    <td className="text-slate-300 text-xs whitespace-nowrap">{fmtDate(tx.txDate)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div className="card overflow-hidden">
          <div className="px-5 py-4 border-b border-zinc-800">
            <h2 className="font-semibold text-slate-200 text-sm">Top Selling Products</h2>
          </div>
          <div className="p-4 space-y-3">
            {topProducts.length === 0 && <p className="text-slate-400 text-sm text-center py-6">No sales yet</p>}
            {topProducts.map((t, i) => (
              <div key={t.stockId} className="flex items-center gap-3">
                <span className="text-slate-400 font-mono text-xs w-4">{i + 1}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-sm text-slate-200 truncate">{t.stock?.ourNo || '—'}</div>
                  <div className="text-xs text-slate-300 truncate">{t.stock?.description}</div>
                </div>
                <div className="text-right shrink-0">
                  <div className="text-xs text-slate-300">{t._sum.quantity} sold</div>
                </div>
              </div>
            ))}
          </div>
          <div className="px-5 pb-4 pt-2 border-t border-zinc-800">
            <Link href="/sales" className="text-accent text-xs hover:text-cyan-300 transition-colors">All sales →</Link>
          </div>
        </div>
      </div>
      ...
      <div className="mt-6 grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { href: '/customers', label: 'Customers',       icon: '◎', color: 'border-zinc-700 hover:border-zinc-500' },
        ].map(a => (
          <Link key={a.href} href={a.href}
            className={`flex items-center gap-3 p-4 rounded-xl bg-zinc-900 border transition-colors ${a.color}`}>
            <span className="text-2xl text-slate-300">{a.icon}</span>
